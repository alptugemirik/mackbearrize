/**
 * QRCode.js v1.0.0 (Pure JS QR Code Generator - Auto Version Scaling)
 */
var QRCode;
(function () {
	...
	QRCodeModel.createData = function (typeNumber, errorCorrectLevel, dataList) {
		...
		for (var i = 0; i < dataList.length; i++) {
			...
			buffer.put(data.getLength(buffer), QRUtil.getLengthInBits(data.mode, typeNumber));
			data.write(buffer);
		}
	...
	QRCode = function (element, option) {
		this._htOption = {
			width: 256,
			height: 256,
			colorDark: "#000000",
		...
		if (option) {
			for (var i in option) this._htOption[i] = option[i];
		}
	...
	QRCode.prototype.makeCode = function (sText) {
		// Otomatik versiyon/kapasite boyutlama döngüsü
		for (var typeNum = 1; typeNum <= 10; typeNum++) {
			try {
				var tc = new QRCodeModel(typeNum, this._htOption.correctLevel);
				tc.addData(sText);
				tc.make();
				this._oQRCode = tc;
				break;
